<div class="catalog-content-wrap conditer-bread-page">
    <div class="mbox">
        <div class="catalog-content">
            <div class="catalog-item-title">
                <span>Хлебо-булочные изделия</span>
            </div>
            <div class="catalog-items cfix">
                <div class="row3">
                    <div class="catalog-item oneHeight large-item">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_1.jpg);"></span>
                                </a>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>Хлеб домашний бабушкин</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>90 P</b> / 350 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                    <div class="catalog-item oneHeight">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_2.jpg);"></span>
                                </a>
                            </div>
                            <div class="red_circle">
                                <div class="red_circle_wrap">
                                    <span>New</span>
                                </div>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>Хлеб 9 злаков</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>99 P</b> / 110 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                    <div class="catalog-item oneHeight">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_3.jpg);"></span>
                                </a>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>Бородинский</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>129 P</b> / 100 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                </div>
                <div class="row4">
                    <div class="catalog-item oneHeight">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_4.jpg);"></span>
                                </a>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>хлеб отрубной ржаной</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>159 P</b> / 100 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                    <div class="catalog-item oneHeight">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_5.jpg);"></span>
                                </a>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>Французский хлеб</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>99 P</b> / 100 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                    <div class="catalog-item oneHeight">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_6.jpg);"></span>
                                </a>
                            </div>
                            <div class="red_circle">
                                <div class="red_circle_wrap">
                                    <span>20%</span>
                                </div>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>Хлеб тостерный</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>499 P</b> / 400 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                    <div class="catalog-item oneHeight">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_7.jpg);"></span>
                                </a>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>хлеб братиславский</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>149 P</b> / 100 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                </div>
                <div class="row4">
                    <div class="catalog-item oneHeight">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_8.jpg);"></span>
                                </a>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>Булачки к завтраку</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>79 P</b> / 100 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                    <div class="catalog-item oneHeight">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_9.jpg);"></span>
                                </a>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>немецкий хлеб</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>399 P</b> / 100 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                    <div class="catalog-item oneHeight">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_10.jpg);"></span>
                                </a>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>Французский багет</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>79 P</b> / 100 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                    <div class="catalog-item oneHeight">
                        <div class="catalog-item-img">
                            <div class="catalog-item-img-wrap">
                                <a href="#catalog_pop" class="loop">
                                    <span style="background-image: url(images/bread_item_11.jpg);"></span>
                                </a>
                            </div>
                        </div>
                        <div class="catalog-item-text">
                            <div class="catalog-item-desc">
                                <p>Батон косичка с тмином</p>
                            </div>
                            <div class="catalog-item-price">
                                <p><b>399 P</b> / 100 <span>гр.</span></p>
                            </div>
                        </div>
                        <div class="add-to-cart">
                            <a href="#catalog_pop" class="catalog_pop">в корзину</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="load-more-items">
                <div class="load-more-wrap cfix">
                    <div class="load-more">
                        <a href="" data-page="bread">Показать ещё</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>