<div class="catalog-content-wrap">
	<div class="mbox">

		<div class="catalog-content">
			<div class="catalog-items cfix">
				<div class="row3">
					<div class="catalog-item oneHeight large-item">
						<div class="catalog-item-img">
							<div class="catalog-item-img-wrap">
								<a href="#catalog_pop" class="loop">
									<span style="background-image: url(images/item1.jpg);"></span>
								</a>
							</div>
							<div class="red_circle">
		                        <div class="red_circle_wrap">
		                            <span>Хит</span>
		                        </div>
		                    </div>
						</div>
						<div class="catalog-item-text">
		                    <div class="catalog-item-desc oneHeight">
		                    	<p>итальянская паста болоньезе</p>
		                    </div>
		                    <div class="catalog-item-price">
		                    	<p><b>279 P</b> / 350 <span>гр.</span></p>
		                    </div>
						</div>
						<div class="add-to-cart">
	                    	<a href="#catalog_pop" class="catalog_pop">в корзину</a>
	                    </div>
					</div>
					<div class="catalog-item oneHeight">
						<div class="catalog-item-img">
							<div class="catalog-item-img-wrap">
								<a href="#catalog_pop" class="loop">
									<span style="background-image: url(images/item2.jpg);"></span>
								</a>
							</div>
							<div class="red_circle">
		                        <div class="red_circle_wrap">
		                            <span>New</span>
		                        </div>
		                    </div>
						</div>
						<div class="catalog-item-text">
		                    <div class="catalog-item-desc oneHeight">
		                    	<p>итальянская паста болоньезе</p>
		                    </div>
		                    <div class="catalog-item-price">
		                    	<p><b>279 P</b> / 350 <span>гр.</span></p>
		                    </div>
						</div>
						<div class="add-to-cart">
	                    	<a href="#catalog_pop" class="catalog_pop">в корзину</a>
	                    </div>
					</div>
					<div class="catalog-item oneHeight">
						<div class="catalog-item-img">
							<div class="catalog-item-img-wrap">
								<a href="#catalog_pop" class="loop">
									<span style="background-image: url(images/item3.jpg);"></span>
								</a>
							</div>
							<div class="red_circle">
		                        <div class="red_circle_wrap">
		                            <span>30%</span>
		                        </div>
		                    </div>
						</div>
						<div class="catalog-item-text">
		                    <div class="catalog-item-desc oneHeight">
		                    	<p>итальянская паста болоньезе</p>
		                    </div>
		                    <div class="catalog-item-price">
		                    	<p><b>279 P</b> / 350 <span>гр.</span></p>
		                    </div>
						</div>
						<div class="add-to-cart">
	                    	<a href="#catalog_pop" class="catalog_pop">в корзину</a>
	                    </div>
					</div>
				</div>
				<div class="row4">
					<div class="catalog-item oneHeight">
						<div class="catalog-item-img">
							<div class="catalog-item-img-wrap">
								<a href="#catalog_pop" class="loop">
									<span style="background-image: url(images/item4.jpg);"></span>
								</a>
							</div>
						</div>
						<div class="catalog-item-text">
		                    <div class="catalog-item-desc oneHeight">
		                    	<p>итальянская паста болоньезе</p>
		                    </div>
		                    <div class="catalog-item-price">
		                    	<p><b>279 P</b> / 350 <span>гр.</span></p>
		                    </div>
						</div>
						<div class="add-to-cart">
	                    	<a href="#catalog_pop" class="catalog_pop">в корзину</a>
	                    </div>
					</div>
					<div class="catalog-item oneHeight">
						<div class="catalog-item-img">
							<div class="catalog-item-img-wrap">
								<a href="#catalog_pop" class="loop">
									<span style="background-image: url(images/item5.jpg);"></span>
								</a>
							</div>
						</div>
						<div class="catalog-item-text">
		                    <div class="catalog-item-desc oneHeight">
		                    	<p>итальянская паста болоньезе</p>
		                    </div>
		                    <div class="catalog-item-price">
		                    	<p><b>279 P</b> / 350 <span>гр.</span></p>
		                    </div>
						</div>
						<div class="add-to-cart">
	                    	<a href="#catalog_pop" class="catalog_pop">в корзину</a>
	                    </div>
					</div>
					<div class="catalog-item oneHeight">
						<div class="catalog-item-img">
							<div class="catalog-item-img-wrap">
								<a href="#catalog_pop" class="loop">
									<span style="background-image: url(images/item6.jpg);"></span>
								</a>
							</div>
							<div class="red_circle">
		                        <div class="red_circle_wrap">
		                            <span>20%</span>
		                        </div>
		                    </div>
						</div>
						<div class="catalog-item-text">
		                    <div class="catalog-item-desc oneHeight">
		                    	<p>итальянская паста болоньезе</p>
		                    </div>
		                    <div class="catalog-item-price">
		                    	<p><b>279 P</b> / 350 <span>гр.</span></p>
		                    </div>
						</div>
						<div class="add-to-cart">
	                    	<a href="#catalog_pop" class="catalog_pop">в корзину</a>
	                    </div>
					</div>
					<div class="catalog-item oneHeight">
						<div class="catalog-item-img">
							<div class="catalog-item-img-wrap">
								<a href="#catalog_pop" class="loop">
									<span style="background-image: url(images/item7.jpg);"></span>
								</a>
							</div>
						</div>
						<div class="catalog-item-text">
		                    <div class="catalog-item-desc oneHeight">
		                    	<p>итальянская паста болоньезе</p>
		                    </div>
		                    <div class="catalog-item-price">
		                    	<p><b>279 P</b> / 350 <span>гр.</span></p>
		                    </div>
						</div>
						<div class="add-to-cart">
	                    	<a href="#catalog_pop" class="catalog_pop">в корзину</a>
	                    </div>
					</div>
				</div>
				<div class="row3">
					<div class="catalog-item oneHeight">
						<div class="catalog-item-img">
							<div class="catalog-item-img-wrap">
								<a href="#catalog_pop" class="loop">
									<span style="background-image: url(images/item8.jpg);"></span>
								</a>
							</div>
						</div>
						<div class="catalog-item-text">
		                    <div class="catalog-item-desc oneHeight">
		                    	<p>итальянская паста болоньезе</p>
		                    </div>
		                    <div class="catalog-item-price">
		                    	<p><b>279 P</b> / 350 <span>гр.</span></p>
		                    </div>
						</div>
						<div class="add-to-cart">
	                    	<a href="#catalog_pop" class="catalog_pop">в корзину</a>
	                    </div>
					</div>
					<div class="catalog-item oneHeight">
						<div class="catalog-item-img">
							<div class="catalog-item-img-wrap">
								<a href="#catalog_pop" class="loop">
									<span style="background-image: url(images/item1.jpg);"></span>
								</a>
							</div>
						</div>
						<div class="catalog-item-text">
		                    <div class="catalog-item-desc oneHeight">
		                    	<p>итальянская паста болоньезе</p>
		                    </div>
		                    <div class="catalog-item-price">
		                    	<p><b>279 P</b> / 350 <span>гр.</span></p>
		                    </div>
						</div>
	                    <div class="add-to-cart">
	                    	<a href="#catalog_pop" class="catalog_pop">в корзину</a>
	                    </div>
					</div>
					<div class="catalog-item oneHeight large-item action-item">
						<div class="catalog-item-sub-img">
							<img src="images/smuzi.png" alt="">
						</div>
						<div class="catalog-item-img">
							<div class="catalog-item-img-wrap">
								<a href="#catalog_pop" class="loop">
									<img src="images/item10-new.jpg" alt=""/>
								</a>
							</div>
						</div>
						<div class="catalog-item-text">
		                    <div class="catalog-item-desc oneHeight">
		                    	<p>Смузи из натуральной черники</p>
		                    </div>
		                    <div class="catalog-item-price">
		                    	<p><b>499 P</b> / 400 <span>гр.</span></p>
		                    </div>
		                    <div class="action-text">
		                    	<p>На данное предложение действует акция “Второй бесплатно”</p>
		                    </div>
						</div>
						<div class="add-to-cart">
	                    	<a href="#catalog_pop" class="catalog_pop">в корзину</a>
	                    </div>
					</div>
				</div>
			</div>
			<div class="load-more-items">
				<div class="load-more-wrap cfix">
					<div class="load-more">
						<a href="#" data-page="catalog">Показать ещё</a>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>